...
public struct PaddleTheSideSnapshotData : ISnapshotData<PaddleTheSideSnapshotData>
{
    public uint tick;
    private int PaddleMoveableComponentPlayerId;
    ...
    public int GetPaddleMoveableComponentPlayerId(GhostDeserializerState deserializerState)
    {
        return (int)PaddleMoveableComponentPlayerId;
    }
    public int GetPaddleMoveableComponentPlayerId()
    {
        return (int)PaddleMoveableComponentPlayerId;
    }
    ...
    public void SetPaddleMoveableComponentPlayerId(int val)
    {
        PaddleMoveableComponentPlayerId = (int)val;
    }
    ...
    public void PredictDelta(uint tick, ref PaddleTheSideSnapshotData baseline1, ref PaddleTheSideSnapshotData baseline2)
    {
    ...
    public void Serialize(int networkId, ref PaddleTheSideSnapshotData baseline, ref DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (PaddleMoveableComponentPlayerId != baseline.PaddleMoveableComponentPlayerId) ? 1u : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedIntDelta(PaddleMoveableComponentPlayerId, baseline.PaddleMoveableComponentPlayerId, compressionModel);
    ...
    public void Deserialize(uint tick, ref PaddleTheSideSnapshotData baseline, ref DataStreamReader reader,
        NetworkCompressionModel compressionModel)
    {
        this.tick = tick;
        changeMask0 = reader.ReadPackedUIntDelta(baseline.changeMask0, compressionModel);
        if ((changeMask0 & (1 << 0)) != 0)
            PaddleMoveableComponentPlayerId = reader.ReadPackedIntDelta(baseline.PaddleMoveableComponentPlayerId, compressionModel);
        else
            PaddleMoveableComponentPlayerId = baseline.PaddleMoveableComponentPlayerId;
        if ((changeMask0 & (1 << 1)) != 0)
        {
    ...
    public void Interpolate(ref PaddleTheSideSnapshotData target, float factor)
    {
