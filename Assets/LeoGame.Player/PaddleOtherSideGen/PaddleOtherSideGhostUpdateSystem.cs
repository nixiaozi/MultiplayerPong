...
public class PaddleOtherSideGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#pragma warning restore 649
#endif
        [ReadOnly] public ArchetypeChunkBufferType<PaddleOtherSideSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<PaddleMoveableComponent> ghostPaddleMoveableComponentType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostPaddleMoveableComponentArray = chunk.GetNativeArray(ghostPaddleMoveableComponentType);
            ...
            for (int entityIndex = 0; entityIndex < ghostEntityArray.Length; ++entityIndex)
            {
                ...
                PaddleOtherSideSnapshotData snapshotData;
                if (!snapshot.GetDataAtTick(targetTick, targetTickFraction, out snapshotData))
                    return;

                var ghostPaddleMoveableComponent = ghostPaddleMoveableComponentArray[entityIndex];
                var ghostRotation = ghostRotationArray[entityIndex];
                var ghostTranslation = ghostTranslationArray[entityIndex];
                ghostPaddleMoveableComponent.PlayerId = snapshotData.GetPaddleMoveableComponentPlayerId(deserializerState);
                ...
                ghostPaddleMoveableComponentArray[entityIndex] = ghostPaddleMoveableComponent;
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<PaddleOtherSideSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<PaddleMoveableComponent>(),
        ...
        {
            All = new []{
                ComponentType.ReadOnly<PaddleOtherSideSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
        ...
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<PaddleOtherSideSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<PaddleOtherSideSnapshotData>(true),
                ...
                ghostPaddleMoveableComponentType = GetArchetypeChunkComponentType<PaddleMoveableComponent>(),
...
public partial class PaddleOtherSideGhostSpawnSystem : DefaultGhostSpawnSystem<PaddleOtherSideSnapshotData>
{
    struct SetPredictedDefault : IJobParallelFor
    {
        [ReadOnly] public NativeArray<PaddleOtherSideSnapshotData> snapshots;
        ...
        {
            if (localPlayerId.Length == 1 && snapshots[index].GetPaddleMoveableComponentPlayerId() == localPlayerId[0].Value)
                predictionMask[index] = 1;
        }
    }
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<PaddleOtherSideSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
